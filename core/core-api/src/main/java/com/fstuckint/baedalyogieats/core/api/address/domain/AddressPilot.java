...
public class AddressPilot {
    ...
    public AddressResult registerAddress(String token, AddressRequest addressRequest) {
        ...
        UserEntity useEntity = userRepository.findByUuidAndIsDeletedFalse(userUuid).orElseThrow(() -> new AddressException(ErrorType.NOT_FOUND_ERROR));
        Address address = addressRequest.toAddress();
        if (useEntity.getUuid() != addressRequest.userUuid()) throw new AddressException(ErrorType.BAD_REQUEST_ERROR);
    ...
    public List<AddressResult> getAddressListByUser(String token, UUID userUuid) {
        ...
        if (uuid != userUuid) throw new AddressException(ErrorType.TOKEN_ERROR);
        return addressRepository.findByUserUuidAndIsDeletedFalse(uuid)
                .stream()
                .map(AddressResult::of)
                .toList();
    }
    ...
    public void checkIdentity(String token, UUID addressId) {
        ...
        UUID userUuid = UUID.fromString(jwtUtils.extractClaims(token).get(JwtUtils.CLAIMS_UUID).toString());
        if (addressEntity.getUserUuid() != userUuid) throw new AddressException(ErrorType.TOKEN_ERROR);
    }
