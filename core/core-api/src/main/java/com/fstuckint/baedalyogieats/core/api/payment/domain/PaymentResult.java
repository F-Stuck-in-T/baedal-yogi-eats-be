...
public record PaymentResult(UUID paymentUuid, UUID orderUuid, UUID userUuid, UUID storeUuid, PaymentType paymentType, Integer amount) {

    public static PaymentResult of(PaymentEntity paymentEntity) {
        return new PaymentResult(
                paymentEntity.getUuid(),
                paymentEntity.getOrderUuid(),
                paymentEntity.getUserUuid(),
                paymentEntity.getStoreUuid(),
                paymentEntity.getStatus(),
                paymentEntity.getAmount());
    }

    public static PaymentPageResponse of(Page<PaymentEntity> pageEntity) {
        List<PaymentResult> pageResult = pageEntity.stream()
